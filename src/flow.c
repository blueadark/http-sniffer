	...
	if (fin_seq != NULL && seq != NULL){
		...
		while(compare_sequence_time(seq, fin_seq) < 0){
			...
			if(pkt != NULL && pkt->http == HTTP_REQ){
				...
				http_parse_request(req,pkt->tcp_odata, pkt->tcp_odata + pkt->tcp_dl, pkt->http_odata, pkt->http_odata+pkt->http_dl);
			}else{
		...
		while(compare_sequence_time(seq, fin_seq) < 0){
			...
			if ( pkt != NULL && pkt->http == HTTP_RSP){
				...
				if(tmp == NULL)
					...
					http_parse_response(rsp, pkt->tcp_odata, pkt->tcp_odata + pkt->tcp_dl, pkt->http_odata, pkt->http_odata+pkt->http_dl);
				}
			}else{
				if(found_http == NULL){
					seq = seq->next;
